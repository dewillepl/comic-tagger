...
def display_volume_details(volume_data):
    ...
    if volume_data.get('image') and volume_data['image'].get('small_url'):
...
def handle_fetch_comicvine(args):
    """
    Fetches data from ComicVine, displays it, and returns the raw data for further processing.
    """
    params = {} 
    ...
    if getattr(args, 'get_issue', None):
        ...
        display_issue_details_summary(issue_details)
        return issue_details

    elif getattr(args, 'get_volume', None):
        url = f"{CV_BASE_URL}volume/{CV_VOLUME_PREFIX}{args.get_volume}/"
        params['field_list'] = 'id,name,issues,publisher,start_year,count_of_issues,description,image,site_detail_url'
        api_data = make_comicvine_api_request(url, params)
        if not (api_data and api_data.get('results')):
    ...
    elif (getattr(args, 'cv_name_filter', None) or getattr(args, 'cv_author_name', None)):
        ...
        params['field_list'] = 'id,name,publisher,start_year,count_of_issues,site_detail_url,people'
        params['limit'] = 25
        params['sort'] = 'name:asc'
        
        print_info(f"Searching ComicVine volumes with filters: {params.get('filter', 'None')}")
        api_data = make_comicvine_api_request(url, params)
        if not (api_data and api_data.get('results')):
        ...
        if getattr(args, 'cv_start_year', None) is not None:
